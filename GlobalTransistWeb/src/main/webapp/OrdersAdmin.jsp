...
<body>
 			...
 			%>
 					<div class="bg-warning p-2 m-2">
 						<p>
 							...
 							%>
 								<a href="ChangeOrderStatus?id=<%= order.get("id") %>&email=<%= userEmail %>&status=Accepted&page=OrdersAdmin" class="btn btn-sm btn-success mx-2">Accept</a>
 								<a href="ChangeOrderStatus?id=<%= order.get("id") %>&email=<%= userEmail %>&status=Rejected&page=OrdersAdmin" class="btn btn-sm btn-danger mx-2">Reject</a>
 							<%	
 							}
 							if(status.equalsIgnoreCase("Accepted")){
 	 						%>
 	 							<a href="ChangeOrderStatus?id=<%= order.get("id") %>&email=<%= userEmail %>&status=Delivered&page=OrdersAdmin" class="btn btn-sm btn-success mx-2">Delivered</a>
 	 						<%	
 	 						}
 							%>
 						</p>
    ...
    <footer class="bg-dark p-5 text-center">
        <a href="index.html">
          <img src="resources/icon_transmit_icon-icons.com_54870.png" height="35px" height="30px" alt="">
